...
class RlsIntegrationTest {
    ...
    private fun insertExampleTask(name: String, description: String, status: String) {
        val id = UUID.randomUUID()
        val sql = """
    ...
    fun `test INSERT with RLS - should prevent inserting with incorrect tenant_id`() {
        ...
        assertThrows<DataIntegrityViolationException> {
            jdbcTemplate.update("""
                INSERT INTO example_notes (id, tenant_id, title, content, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?)
            """.trimIndent(), noteId, tenant2Id, "Cross-Tenant Insert Attempt", "This should fail", now, now)
        }
        ...
        jdbcTemplate.update("""
            INSERT INTO example_notes (id, tenant_id, title, content, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
        """.trimIndent(), validNoteId, tenant1Id, "Valid Insert", "This should succeed", now, now)
    ...
    fun `test cross-tenant JOIN - should only return data for current tenant`() {
        ...
        jdbcTemplate.update("""
            INSERT INTO example_notes (id, tenant_id, title, content, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
        """.trimIndent(), noteId, tenant1Id, "Related Note", "Note related to task", now, now)
        
        jdbcTemplate.update("""
            INSERT INTO example_tasks (id, tenant_id, name, description, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """.trimIndent(), taskId, tenant1Id, "Related Task", "Task related to note", "OPEN", now, now)
        ...
        jdbcTemplate.update("""
            INSERT INTO example_notes (id, tenant_id, title, content, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
        """.trimIndent(), noteId2, tenant2Id, "Related Note T2", "Note related to task T2", now, now)
        
        jdbcTemplate.update("""
            INSERT INTO example_tasks (id, tenant_id, name, description, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """.trimIndent(), taskId2, tenant2Id, "Related Task T2", "Task related to note T2", "OPEN", now, now)
    ...
    fun `test BYPASSRLS - admin role should be able to see all tenant data`() {
        ...
        assertEquals("Admin Cross-Tenant Update", updatedNoteTitle, 
