...
class TenantCoroutineContextTest {
    ...
    @Test
    fun `should propagate tenant ID in coroutine context`() = runTest {
        // Given
    ...
    @Test
    fun `should not propagate tenant ID without explicit context element`() = runTest {
        // Given
        val tenantId = UUID.randomUUID()
        TenantContextHolder.setTenantId(tenantId)
    ...
    @Test
    fun `should propagate tenant ID in nested coroutines`() = runTest {
        // Given
        val tenantId = UUID.randomUUID()
        TenantContextHolder.setTenantId(tenantId)
        val tenantContext = TenantCoroutineContext.capture()
    ...
    @Test
    fun `should capture null tenant ID when none is set`() = runTest {
        // Given
        // No tenant ID set
    ...
    @Test
    fun `should restore previous tenant ID after coroutine execution`() = runTest {
        // Given
    ...
    @Test
    fun `should work with multiple concurrent coroutines`() = runBlocking {
        // Given
    ...
    @Test
    fun `should propagate tenant ID to async operations`() = runTest {
        // Given
        val tenantId = UUID.randomUUID()
        TenantContextHolder.setTenantId(tenantId)
        val tenantContext = TenantCoroutineContext.capture()
