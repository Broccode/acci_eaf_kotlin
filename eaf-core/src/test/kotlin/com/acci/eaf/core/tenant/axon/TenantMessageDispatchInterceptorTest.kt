...
class TenantMessageDispatchInterceptorTest {
    ...
    fun setUp() {
        ...
        lenient().`when`(message.payloadType).thenReturn(String::class.java)
        
        // Einfache Stub-Konfiguration für alle Tests
        lenient().`when`(message.withMetaData(any())).thenReturn(resultMessage)
    }
    ...
    fun `should add tenant ID to message metadata when tenant context is set`() {
        ...
        `when`(resultMessage.metaData).thenReturn(expectedMetadata)
        ...
        verify(message).withMetaData(argThat { metadata: Map<String, Any> ->
    ...
    fun `should not add tenant ID to message metadata when tenant context is not set`() {
        ...
        `when`(message.metaData).thenReturn(emptyMetadata)
        ...
        verify(message, times(0)).withMetaData(any())
    }
} 